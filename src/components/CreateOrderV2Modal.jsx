...
function OrderPreviewV2({type,customerName,customerPhone,selectedSchool,items,guestCount,fulfillment,address,requiredAt,note,itemsTotal,shipFee,paymentMethod,deposit,grandTotal,remaining}){
 const itemLine=(it)=>{
  ...
  const parts=[s.size,s.cot,fillingLabel,s.content&&`chữ: ${s.content}`,s.candle&&`nến: ${s.candle}`,s.packing,s.color,s.flavor,s.spec,s.catalog_specification].filter(Boolean);
   ...
   {items.map((it)=>{const itemIndex=items.indexOf(it);const itemPrice=getPrice(it);const itemTotal=getItemTotal(it);return <div key={it.id} style={{padding:12,border:'1px solid var(--border-default)',borderRadius:16,marginBottom:10,background:'var(--surface-card)'}}>
    ...
    <div className="sumi-item-fields" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginTop:8}}>
     ...
     {(it.flow_type||type)==='macaron'&&<>
      ...
      <input style={fieldStyle} placeholder="Quy cách (VD: Khay 36 cặp - 4cm, Hộp 100 cặp - 2cm)" value={it.specification?.packing||''} onChange={e=>spec(itemIndex,'packing',e.target.value)}/>
      <input style={fieldStyle} placeholder="Màu / phối màu (VD: Full màu nâu, hoặc Mix 12 màu - đỏ 4 khay, xanh 6 khay...)" value={it.specification?.color||''} onChange={e=>spec(itemIndex,'color',e.target.value)}/>
      <input style={fieldStyle} placeholder="Vị / có nhân" value={it.specification?.flavor||''} onChange={e=>spec(itemIndex,'flavor',e.target.value)}/>
     </>}
