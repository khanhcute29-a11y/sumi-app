...
export default function ExportSummaryModal({ mode, onClose }) {
  ...
  const [format, setFormat] = useState('xlsx'); // Đơn hàng chỉ xuất Excel; Doanh thu còn tuỳ chọn CSV
  ...
  const run = async () => {
    ...
    setBusy(true); setMsg('');
    try {
      const n = await (isOrders ? exportOrdersSummary(range.from, range.to, flows) : exportRevenueSummary(range.from, range.to, format, flows));
      setMsg(n ? `Đã xuất ${isOrders ? `${n} đơn hàng` : `${n} dòng dữ liệu`} (${isOrders ? '1 file Excel' : (format === 'xlsx' ? '1 file Excel, 3 sheet' : '3 file CSV')}).` : 'Không có dữ liệu trong khoảng này — file chỉ có tiêu đề.');
    } catch (err) {
