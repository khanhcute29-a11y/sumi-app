...
export function AccountantOverviewV1Inner() {
  ...
  const monthKey = useMemo(() => monthKeyOf(), []);
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      setPendingDirector(directorTotals);
      setLedgerRows(ledger);
      setWages(wagesSummary);
    } catch (e: any) {
  ...
  const sheetDragHandlers = {
    onTouchStart: (e: React.TouchEvent) => { dragStartY.current = e.touches[0].clientY; setIsDragging(true); },
    onTouchMove: (e: React.TouchEvent) => { const d = e.touches[0].clientY - dragStartY.current; if (d > 0) setDragY(d); },
  ...
  const totalReadyToPay = readyExpenses.reduce((s, c) => s + (Number(c.amount) || 0), 0) + readyAdvances.reduce((s, a) => s + (Number(a.amount) || 0), 0);
                  ...
                  <div key={exp.id} style={{ background: '#fff', border: '1.5px solid #eadcca', borderRadius: 16, padding: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontSize: 13.5, fontWeight: 900, color: '#2d1c10' }}>{exp.description}</div>
                        <div style={{ fontSize: 11, color: '#725f50', marginTop: 2 }}>{exp.claimant_name}{exp.approval_reason ? ` · ${exp.approval_reason}` : ''}</div>
                      </div>
                ...
                {ledgerRows.map((r: any) => (
                  <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#fff', border: '1px solid #eadcca', borderRadius: 12, padding: '10px 12px' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12.5, fontWeight: 800, color: '#2d1c10', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.label}</div>
                      <div style={{ fontSize: 10.5, color: '#8c7664' }}>{new Date(r.occurred_at).toLocaleString('vi-VN')}</div>
                    </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {wages.rows.map((row: any) => (
                  <div key={row.employeeId} style={{ background: '#fff', border: '1px solid #eadcca', borderRadius: 14, padding: 12 }}>
                    <div style={{ fontSize: 13.5, fontWeight: 900, color: '#2d1c10' }}>{row.employeeName}</div>
