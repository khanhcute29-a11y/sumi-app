...
export async function fetchWagesSummaryForMonth(monthKey = monthKeyOf()) {
  ...
  const ensure = (id, name) => {
    if (!byEmployee[id]) byEmployee[id] = { employeeId: id, employeeName: name, advancePaid: 0, expensesRecorded: 0, netPay: null };
    return byEmployee[id];
  };
  (paidAdvancesRes.data || []).forEach((a) => { ensure(a.employee_id, a.employee_name).advancePaid += Number(a.amount) || 0; });
  (recordedClaimsRes.data || []).forEach((c) => { ensure(c.claimant_id, c.claimant_name).expensesRecorded += Number(c.amount) || 0; });
  entries.forEach((e) => {
    const row = byEmployee[e.employee_id];
    if (row) row.netPay = Number(e.base_pay || 0) + Number(e.overtime_pay || 0) + Number(e.allowance || 0) + Number(e.kpi_bonus || 0) + Number(e.output_bonus || 0) + Number(e.delegation_bonus || 0) + Number(e.other_bonus || 0) - Number(e.advance_amount || 0) - Number(e.deduction_amount || 0);
  });
