...
function Rows({rows,kind,director,finance,busy,act}){return <section className="finance-list"><div className="finance-list-title"><h2>{kind==='expense'?'Các khoản đã báo':'Yêu cầu tạm ứng'}</h2><b>{rows.filter(x=>x.status.startsWith('pending')).length} chờ</b></div>{rows.length===0?<p className="finance-empty">Chưa có dữ liệu.</p>:rows.map(r=><article key={r.id} className={`finance-row ${r.status}`}><div className="finance-row-top"><div><strong>{kind==='expense'?r.description:r.employee_name}</strong><small>{kind==='expense'?r.claimant_name:r.reason}</small></div><b>{money(r.amount)}</b></div><div className="finance-meta"><span>{labels[r.status]}</span><time>{new Date(r.occurred_at||r.created_at).toLocaleString('vi-VN')}</time></div>{kind==='expense'&&r.note&&<p>Ghi chú: {r.note}</p>}{kind==='expense'&&r.approval_reason&&<em>⚠ {r.approval_reason}</em>}{kind==='expense'&&r.receipt_attachments?.[0]?.url&&<a href={r.receipt_attachments[0].url} target="_blank" rel="noreferrer">📷 Xem chứng từ</a>}{r.status==='pending_director'&&director&&<div className="finance-actions"><button onClick={()=>act(kind==='expense'?'review_expense_claim':'review_salary_advance',{p_id:r.id,p_approve:false,p_note:null})}>Từ chối</button><button onClick={()=>act(kind==='expense'?'review_expense_claim':'review_salary_advance',{p_id:r.id,p_approve:true,p_note:null})}>Duyệt</button></div>}{r.status==='pending_accounting'&&finance&&<button className="finance-record" disabled={busy} onClick={()=>act(kind==='expense'?'record_expense_claim':'pay_salary_advance',{p_id:r.id})}>{kind==='expense'?'XÁC NHẬN & GHI SỔ':'XÁC NHẬN ĐÃ CHI'}</button>}</article>)}</section>}
