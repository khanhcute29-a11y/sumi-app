...
export default function MobileProfileScreen({onSignOut,onNavigate}){
 ...
 return <div className="sumi-profile-page">
  ...
  <div className="sumi-section-head"><span>CÔNG VIỆC & THU NHẬP</span></div><button className="sumi-menu-button" onClick={()=>onNavigate?.('shifts')}><span>⏱️</span><b>Chấm công và lịch làm</b><em>›</em></button><button className="sumi-menu-button" onClick={()=>onNavigate?.('compensation')}><span>💵</span><b>Tăng ca và lương tháng</b><em>›</em></button><button className="sumi-menu-button" onClick={()=>onNavigate?.('financeRequests')}><span>🧾</span><b>Chi & tạm ứng</b><em>›</em></button>{isDirector&&<button className="sumi-menu-button" onClick={()=>onNavigate?.('approvals')}><span>✅</span><b>Yêu Cầu Duyệt</b><em>›</em></button>}
