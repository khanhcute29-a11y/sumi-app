...
import { IconClock, IconMoney, IconReceipt, IconReports, IconCheck, IconSettings, IconBell, IconImage, IconLogout } from '../components/icons/FrogIcons';
...
export default function MobileProfileScreen({onSignOut,onNavigate}){
 ...
 return <div className="sumi-profile-page">
  ...
  <div className="sumi-section-head"><span>CÔNG VIỆC & THU NHẬP</span></div><button className="sumi-menu-button" onClick={()=>onNavigate?.('shifts')}><span><IconClock size={20}/></span><b>Chấm công và lịch làm</b><em>›</em></button><button className="sumi-menu-button" onClick={()=>onNavigate?.('compensation')}><span><IconMoney size={20}/></span><b>Tăng ca và lương tháng</b><em>›</em></button><button className="sumi-menu-button" onClick={()=>onNavigate?.('financeRequests')}><span><IconReceipt size={20}/></span><b>Chi & tạm ứng</b><em>›</em></button>{isFinance&&<button className="sumi-menu-button" onClick={()=>onNavigate?.('accountantOverview')}><span><IconReports size={20}/></span><b>Kế toán tổng quan</b><em>›</em></button>}{isDirector&&<button className="sumi-menu-button" onClick={()=>onNavigate?.('approvals')}><span><IconCheck size={20}/></span><b>Yêu Cầu Duyệt</b><em>›</em></button>}
