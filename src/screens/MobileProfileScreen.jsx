...
const roles={owner:'Giám đốc Kinh doanh',admin:'Quản lý',accountant:'Kế toán',cashier:'Thu ngân',kitchen_lead:'Bếp trưởng',kitchen_deputy:'Bếp phó',kitchen:'Thợ làm bánh',bakery:'Nhân viên Bakery',shipper:'Nhân viên vận tải',warehouse:'Nhân viên kho'};
...
export default function MobileProfileScreen({onSignOut,onNavigate}){
 ...
 return <div className="sumi-profile-page">
  <div className="sumi-profile-card"><button className="sumi-profile-avatar" onClick={()=>inputRef.current?.click()} aria-label="Đổi ảnh đại diện"><UserAvatar profile={profile} size={76}/><em>📷</em></button><input ref={inputRef} hidden type="file" accept="image/*" capture="user" onChange={e=>changeAvatar(e.target.files?.[0])}/><div><h1>{profile?.full_name||'Nhân viên SUMI'}</h1><p>{roles[profile?.role]||profile?.role}{profile?.station?` · ${profile.station}`:''}</p><small>{uploading?'Đang tải ảnh...':'Chạm vào ảnh để thay đổi'}</small></div></div>
  ...
  <div className="sumi-section-head"><span>CÔNG VIỆC & THU NHẬP</span></div><button className="sumi-menu-button" onClick={()=>onNavigate?.('shifts')}><span>⏱️</span><b>Chấm công và lịch làm</b><em>›</em></button><button className="sumi-menu-button" onClick={()=>onNavigate?.('compensation')}><span>💵</span><b>Tăng ca và lương tháng</b><em>›</em></button><button className="sumi-menu-button" onClick={()=>onNavigate?.('financeRequests')}><span>🧾</span><b>Chi & tạm ứng</b><em>›</em></button>
  <div className="sumi-section-head"><span>TÀI KHOẢN</span></div><button className="sumi-menu-button" onClick={()=>onNavigate?.('settings')}><span>⚙️</span><b>Thiết lập tài khoản</b><em>›</em></button><button className="sumi-menu-button" onClick={()=>onNavigate?.('inbox')}><span>🔔</span><b>Thông báo của tôi</b><em>›</em></button><button className="sumi-menu-button"><span>❓</span><b>Hướng dẫn bằng hình ảnh</b><em>›</em></button><button className="sumi-menu-button danger" onClick={onSignOut}><span>🚪</span><b>Đăng xuất</b><em>›</em></button>
 </div>;
}
