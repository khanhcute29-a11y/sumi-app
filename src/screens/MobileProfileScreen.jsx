...
export default function MobileProfileScreen({onSignOut,onNavigate}){
 ...
 return <div className="sumi-profile-page">
  <div className="sumi-profile-card"><button className="sumi-profile-avatar" onClick={()=>inputRef.current?.click()} aria-label="Đổi ảnh đại diện"><UserAvatar profile={profile} size={76}/><em>📷</em></button><input ref={inputRef} hidden type="file" accept="image/*" capture="user" onChange={e=>changeAvatar(e.target.files?.[0])}/><div><h1>{profile?.full_name||'Nhân viên SUMI'}</h1><p>{ROLE_META[profile?.role]?.label||profile?.role}{profile?.station?` · ${profile.station}`:''}</p><small>{uploading?'Đang tải ảnh...':'Chạm vào ảnh để thay đổi'}</small></div></div>
