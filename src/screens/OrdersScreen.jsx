      ...
      {isSizeVariant && (() => {
        ...
        return (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', minWidth: 0 }}>
            ...
            {showCakeExtras && <Input label="Nội dung (chữ trên bánh)" placeholder="VD: Chúc mừng sinh nhật Linh" value={item.content || ''} onChange={(e) => set('content', e.target.value)} style={{ flex: '2 1 220px', minWidth: 0 }} />}
            {showCakeExtras && <Input label="Loại nến" placeholder="VD: Nến số 25" value={item.candle || ''} onChange={(e) => set('candle', e.target.value)} style={{ flex: '1 1 150px', minWidth: 0 }} />}
          </div>
        );
      })()}
      {isQtyOnly && (
      ...
      {modalOrder && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--surface-overlay)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 16 }} onClick={() => setModalOrder(null)}>
          <div style={{ background: 'var(--surface-card)', borderRadius: 'var(--radius-lg)', width: 420, maxWidth: '100%', maxHeight: '90vh', overflowY: 'auto', padding: 20, boxShadow: 'var(--shadow-lg)', display: 'flex', flexDirection: 'column', gap: 10 }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ font: 'var(--text-caption)', color: 'var(--text-muted)' }}>Ngày đặt đơn: {modalOrder.created_at ? new Date(modalOrder.created_at).toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' }) : '—'}</div>
