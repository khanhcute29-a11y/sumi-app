-- create_order_v2 (V2 order creation) chưa từng ghi ship_fee/deposit/payment_method/total,
-- dù các cột này đã có sẵn trên bảng orders từ trước (dùng bởi luồng đơn hàng cũ). Vì vậy
-- đơn tạo qua V2 luôn bị ship_fee=0, deposit=0, payment_method=null, total=0 dù nhân viên
-- đã nhập đầy đủ trên form — báo cáo/đối chiếu công nợ đọc thẳng các cột này sẽ sai.
begin;
...
values('202608260003_add_ship_fee_deposit_payment_to_create_order_v2', 'completed', now(),
  'create_order_v2 now accepts and stores ship_fee, deposit, payment_method and total, matching the fields the legacy order form used to capture.')
