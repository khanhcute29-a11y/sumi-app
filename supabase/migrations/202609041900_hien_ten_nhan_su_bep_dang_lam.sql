-- Fix: sheet "Danh Sách Đơn Hàng" (BossOverviewV3) và thẻ đơn OrdersV2Screen chỉ
-- hiện nhãn trạng thái chung chung "Bếp đang làm 👩‍🍳", không có tên nhân sự nào
-- đang thực hiện đơn đó — phản hồi thật: "Ko có tên nguoi bấm làm". View
-- order_operations_list vốn có kitchen_names (tên BỘ PHẬN, vd "Bếp Lạnh") qua
-- order_work_packages nhưng chưa từng lấy tên NHÂN SỰ.
--
-- Bản đầu của migration này join qua order_work_packages.accepted_by — SAI
-- nguồn: kiểm tra dữ liệu thật (SUMI-20260903-61756) thấy accepted_by luôn
-- null. Lý do: 2 RPC đang thật sự chạy trong app khi bếp nhận việc
-- (accept_work_package_self, accept_delegate_work_package — migration
-- 202608260020) ghi tên vào work_packages.assigned_to_staff_name (cột text
-- có sẵn), KHÔNG set accepted_by. Chỉ RPC accept_order_package cũ hơn mới set
-- accepted_by. Sửa lại: ưu tiên assigned_to_staff_name, fallback qua
-- profiles.full_name (accepted_by) cho đường cũ.
--
-- Cột mới đặt Ở CUỐI danh sách SELECT (CREATE OR REPLACE VIEW không cho đổi
-- thứ tự/xoá cột cũ) — bản đầu tiên còn bị lỗi vì dựa nhầm vào bản view CŨ
-- hơn, thiếu cột customer_phone mà migration 202609042100 đã thêm vào cuối.
begin;
 ...
 SELECT o.id,
   ...
   FROM orders o
     ...
     LEFT JOIN LATERAL ( SELECT string_agg(DISTINCT ou.name, ', '::text) AS kitchen_names,
            ...
            string_agg(DISTINCT COALESCE(wp.assigned_to_staff_name, p2.full_name), ', '::text) AS kitchen_staff_names
           FROM order_work_packages wp
             JOIN organization_units ou ON ou.id = wp.unit_id
             LEFT JOIN profiles p2 ON p2.id = wp.accepted_by
...
values('202609041900_hien_ten_nhan_su_bep_dang_lam', 'completed', now(),
  'Them cot kitchen_staff_names (o cuoi SELECT, khong doi cot cu) vao view order_operations_list, uu tien lay tu order_work_packages.assigned_to_staff_name (fallback profiles.full_name qua accepted_by), de hien ten nhan su dang lam don thay vi chi hien nhan chung chung "Bep dang lam".')
