-- Fix tiếp "column planned_distance_km does not exist" — cột này không hề
-- tồn tại trên bảng orders (đã kiểm tra lại information_schema đúng cách,
-- lần trước viết sai độ ưu tiên AND/OR nên tưởng nhầm là có). Bỏ hẳn nhánh
-- fallback đó, chỉ tính km khi có đủ toạ độ GPS thật (pickup_lat/delivery_lat).
begin;
...
begin
  ...
  if v_role = 'shipper' then
    with don_giao as (
      ...
      where o.shipper_staff_name = v_ten
...
values('202609101330_fix_missing_planned_distance_column', 'completed', now(),
  'Fix "column planned_distance_km does not exist" - cot nay khong ton tai tren orders (kiem tra lai information_schema dung cach), bo han nhanh fallback do, chi tinh km khi co du toa do GPS that.')
